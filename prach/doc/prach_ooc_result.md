...
- Date: 2026-09-13
- Source commit: `41fdd7a` (PRACH RTL unchanged; only `prach/README.md` differed
  in the working tree)
...
| Resource        | Synth | Impl  |
|-----------------|------:|------:|
| CLB LUTs        | 19594 | 17597 |
| — LUT as Logic  |   n/a | 13163 |
| — LUT as Memory |   n/a |  4434 |
| CLB Registers   | 26691 | 25755 |
...
|---|---|---:|---:|---:|---:|---:|
| `prach` | (top) | 17597 | 13163 | 1530 | 2904 | 25755 |
| `g_cc[n].u_channel` (x3) | `prach_channel` | 5675 | 4197 | 510 | 968 | 8290 |
| `u_fft` | `prach_fft` | 2425 | 1965 | 252 | 208 | 3446 |
| `u_ddc` | `prach_ddc` | 1378 | 467 | 194 | 717 | 2114 |
| `u_framer` | `prach_framer` | 1030 | 966 | 64 | 0 | 1573 |
| &nbsp;&nbsp;`u_buffer` | `prach_framer_buffer` | 936 | 872 | 64 | 0 | 1280 |
| &nbsp;&nbsp;&nbsp;&nbsp;`u_compress` | `prach_bfp_compress` | 536 | 536 | 0 | 0 | 936 |
| &nbsp;&nbsp;&nbsp;&nbsp;`u_gearbox` | `prach_bfp_gearbox` | 270 | 270 | 0 | 0 | 278 |
| &nbsp;&nbsp;&nbsp;&nbsp;`u_fifo` | `axis_fifo_alt` | 93 | 93 | 0 | 0 | 293 |
| `u_ctrl` | `prach_ctrl` | 347 | 347 | 0 | 0 | 722 |
| `u_stream2block` | `prach_stream2block` | 332 | 304 | 0 | 28 | 201 |
| `u_resync` | `prach_resync` | 90 | 90 | 0 | 0 | 127 |
| `u_pulse_delay_in` | `pulse_delay` | 72 | 57 | 0 | 15 | 53 |
| `u_symbol_timer` | `symbol_timer` | 40 | 40 | 0 | 0 | 54 |

So per CC: FFT 43 %, DDC 24 %, framer (incl. buffer/compress/gearbox/fifo) 18 %,
control 6 %, stream2block 6 %.
...
### FFT stages (one `u_fft`; FFT total 2425 LUT / 252 LUTRAM / 208 SRL / 3446 FF)
...
| `g_left_dit2[4].g_left.u_ditfft2` | 48 | 204 | 166 | 18 | 20 | 287 |
| `g_left_dit2[5].g_left.u_ditfft2` | 96 | 245 | 189 | 36 | 20 | 291 |
| `g_left_dit2[6].g_left.u_ditfft2` | 192 | 307 | 233 | 72 | 2 | 363 |
| `g_left_dit2[7].g_left.u_ditfft2` | 384 | 361 | 235 | 126 | 0 | 409 |
| `g_left_dit2[8].g_left.u_ditfft2` | 768 | 166 | 166 | 0 | 0 | 341 |
| `g_left_dit2[9].g_left.u_ditfft2` | 1536 | 174 | 174 | 0 | 0 | 345 |
| `u_delay_chn` | 2b x 1622 | 102 | 0 | 0 | 102 | 2 |
| `u_delay_sy` | `pulse_delay` | 44 | 36 | 0 | 8 | 29 |
...
|---|---:|---:|---:|---|
| `prach` (all) | 1530 | 2904 | **4434** | 25 % of the 17597 LUTs |
...
|----------------|----------:|----------:|----------:|----------:|
| `clk`          |  2.035 ns | 491.4 MHz | +0.117 ns | +0.006 ns |
| `clk_eth_xran` |  2.500 ns | 400.0 MHz | +0.206 ns | +0.042 ns |
| `s_axi_aclk`   | 10.000 ns | 100.0 MHz | +6.381 ns | +0.046 ns |
...
by about 0.1 ns, so any datapath change must re-run implementation.
...
- 2026-09-10, commit `639ab21`: 19600 synth / 17607 impl CLB LUTs, 26691 / 25791
  FF. Within tool-run variation of the run above, which is why this entry replaces
  it rather than being listed separately.
