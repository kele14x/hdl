// File: ptp_framer_eth.sv
// Brief: Per Ethernet channel for framer. This module does:
//        - CDC UL packet from clk domain to Ethernet TX clock domain
//        - Add MAC header, including VLAN is enabled
`timescale 1 ns / 1 ps
//
`default_nettype none

module ptp_framer_eth (
    // Tx Ethernet ports
    //------------------
  ...
  logic        mac_with_vlan = 1;

  logic [47:0] mac_dest_mac = 48'h001122334455;
  logic [47:0] mac_source_mac = 48'h001122334466;
  logic [15:0] mac_vlan_type = 16'h8100;
  logic [15:0] mac_vlan_tag = 16'h7001;
  ...
  always_ff @(posedge clk) begin
    if (state == S_DMAC_SMAC0) begin
      m0_axis_tdata <= byte_reverse({mac_dest_mac, mac_source_mac[47:32]});
    end else if (state == S_SMAC1_VLAN) begin
      m0_axis_tdata <= byte_reverse({mac_source_mac[31:0], mac_vlan_type, mac_vlan_tag});
    end else if (state == S_SMAC1_ETYPE) begin
      m0_axis_tdata <= byte_reverse({mac_source_mac[31:0], mac_ethertype, s_axis_tdata_reversed[63:48]});
