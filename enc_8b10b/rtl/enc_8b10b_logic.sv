...
module enc_8b10b_logic (
  ...
  function [9:0] bit_reverse(input [9:0] bin);
    for (int i = 0; i < 10; i = i + 1) bit_reverse[i] = bin[9-i];
  endfunction
 ...
 : dispin;  // D.28.y
  ...
  always_comb begin : c_b4
    case (b3)
      ...
      default:
      b4 = ~(alt || (charisk && kok)) ? (~disp_6b ? 4'b1110 : 4'b0001)  // D.x.P7 / K.err.P7
  ...
  assign enc_result = {charisk && ~kok, disp_4b, bit_reverse({b6, b4})};
