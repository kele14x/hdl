...
module pdxch_fdv_buffer_write #(
  ...
  localparam logic [IQ_STATE_WIDTH-1:0] IQ_TOTAL_LIMIT = IQ_STATE_WIDTH'(2 * IQ_BANK_DEPTH);
  localparam logic [EXP_STATE_WIDTH-1:0] EXP_BANK_LIMIT = EXP_STATE_WIDTH'(EXP_BANK_DEPTH);
  localparam logic [EXP_STATE_WIDTH-1:0] EXP_TOTAL_LIMIT = EXP_STATE_WIDTH'(2 * EXP_BANK_DEPTH);

  logic packet_active;
  logic packet_match_r;
  // Keep the bounds tied to the same bank as the packet's starting address.
  logic packet_bank_r;
  logic [IQ_STATE_WIDTH-1:0] iq_addr_r;
  logic [EXP_STATE_WIDTH-1:0] exp_addr_r;
  logic [11:0] word_count_r;

  logic [IQ_STATE_WIDTH-1:0] wr_iq_addr_c;
  logic wr_iq_en_c;
  logic [35:0] wr_iq_data_c;
  logic [EXP_STATE_WIDTH-1:0] wr_exp_addr_c;
  logic wr_exp_en_c;
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      wr_iq_addr  <= wr_iq_addr_c[IQ_ADDR_WIDTH-1:0];
      wr_iq_en    <= wr_iq_en_c;
      wr_iq_data  <= wr_iq_data_c;
      wr_exp_addr <= wr_exp_addr_c[EXP_ADDR_WIDTH-1:0];
