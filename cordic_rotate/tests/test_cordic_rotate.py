...
def cordic_rotate_model(xin, yin, theta):
    width = DATA_WIDTH + 3
...
async def send_and_check(dut, xin, yin, theta):
    expected = cordic_rotate_model(xin, yin, theta)
    dut.xin.value = xin
    dut.yin.value = yin
...
@cocotb.test()
async def test_cordic_rotate_matches_fixed_point_reference(dut):
    ...
    for vector in ((40, 0, 0b000000), (32, -11, 0b00101), (-20, 17, 0b11010)):
        await send_and_check(dut, *vector)


def test_cordic_rotate_runner():
    ...
    runner.test(
        ...
        test_module="test_cordic_rotate",
        waves=True,
        gui=GUI,
    )
