...
module fh_framer_padding (
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      is_padding <= ~(data_count >= 4'd7);
    end else if ((data_count >= 4'd7) && (~m_axis_tvalid || m_axis_tready)) begin
      is_padding <= 1'b0;
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (~m_axis_tvalid || m_axis_tready) begin
      if (is_padding) begin
        m_axis_tdata <= 64'd0;
        m_axis_tkeep <= (data_count >= 4'd7) ? 8'h0F : 8'hFF;
        m_axis_tlast <= (data_count >= 4'd7);
      end else begin
        m_axis_tdata <= tkeep_null(int_axis_tdata, int_axis_tkeep);
        m_axis_tkeep <= (data_count >= 4'd8) ? int_axis_tkeep :
                        (data_count == 4'd7) ? (int_axis_tkeep | 8'h0F) : 8'hFF;
        m_axis_tlast <= (data_count >= 4'd7) ? int_axis_tlast : 1'b0;
      end
    end
  end
