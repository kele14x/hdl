...
module srs_adaptor_controller_req #(
    ...
    // SRS Request
    output var [          15:0] srs_req_rtc_pc_id,
    output var [           3:0] srs_req_cc,
    output var [          11:0] srs_req_symbol,
    output var [           7:0] srs_req_numprbc,
    output var [           9:0] srs_req_startprbc,
    output var [          11:0] srs_req_sectionid,
    output var [           3:0] srs_req_ethport,
    output var                  srs_req_valid,
    input var                   srs_req_ready
);
  ...
  typedef enum int { S_IDLE, S_ADDR, S_DATA, S_CLR, S_VALID, S_NEXT } state_t;
  ...
  always_comb begin
    case(state)
      ...
      S_VALID: next_state = srs_req_ready ? S_CLR : S_VALID;
      S_CLR:   next_state = S_NEXT;
      S_NEXT:  next_state = ~(rd_addr == {ADDR_WIDTH{1'b1}}) ? S_ADDR : do_another_round ? S_ADDR : S_IDLE;
  ...
  always_ff @(posedge clk) begin
    if (state == S_DATA && process_it) begin
      srs_req_rtc_pc_id <= srs_rtc_pc_id;
      srs_req_cc        <= srs_cc;
      srs_req_symbol    <= srs_symbol;
      srs_req_numprbc   <= srs_numprbc;
      srs_req_startprbc <= srs_startprbc;
      srs_req_sectionid <= srs_sectionid;
      srs_req_ethport   <= srs_ethport;
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      srs_req_valid <= (next_state == S_VALID);
    end
  end
