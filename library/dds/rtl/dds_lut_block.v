...
// Brief: DDS phase-cosine/sine look-up table using block memory resource. When
//        Phase Word Width (PHASE_WIDTH) is large (but not very large), it's
//        more fast to store the waveform in block memory. To save the block
//        memory, it only stores a quarter of the waveform and relies on the
//        trigonometric function to get the correct result.
`timescale 1ns / 1ps
//
