/*
 * LFSR implements a Linear Feedback Shift Register module (LFSR).
 *
 * Features:
 *
 ...
 *
 * Parameters:
 *   BIT_WIDTH       - Width of the LFSR
 *   INITIAL         - Initial state of the LFSR
 *   POLYNOMIAL      - Feedback polynomial
 *   STRUCTURE       - LFSR structure ("FIBONACCI" or "GALOIS")
 *   GATE_TYPE       - Feedback gate type ("XOR" or "XNOR")
 *   PARALLEL_OUTPUT - Enable parallel output if set to 1
 *
 * Ports:
 *   clk   - Clock input
 *   rst   - Reset input
 *   en    - Enable input
 *   load  - Load input (loads din into LFSR when high)
 *   din   - Data input for loading
 *   dout  - Data output (serial or parallel based on PARALLEL_OUTPUT)
 */
...
module lfsr #(
    ...
    parameter reg     [  BIT_WIDTH:0] POLYNOMIAL      = 9'b100000011,
  ...
  initial begin
    if (STRUCTURE != "FIBONACCI" && STRUCTURE != "GALOIS") begin
      $display("LFSR structure (STRUCTURE) should be one of \"FIBONACCI\" or \"GALOIS\". [%m]");
      $finish();
    end

    if (GATE_TYPE != "XOR" && GATE_TYPE != "XNOR") begin
      $display("Gate type (GATE_TYPE) should be one of \"XOR\" or \"XNOR\". [%m]");
      $finish();
    end

    if (POLYNOMIAL[0] != 1'b1 && POLYNOMIAL[BIT_WIDTH] != 1'b1) begin
      $display("Feedback polynomial (POLYNOMIAL) should have MSB & LSB both set to 1. [%m]");
      $finish();
    end
  end
  // verilog_format: on
