# Out-of-context implementation for the mandatory-BFP PUXCH wrapper.
#
# Consumes the synthesis checkpoint written by puxch_ooc.tcl and runs
...
#
# Parameter overrides (defaults match the RTL wrapper: full block + 4k FFT):
...
puts "INFO: puxch OOC impl params: HALF_BLOCK=$half_block HALF_FFT=$half_fft"
set build_dir [file normalize [file join $repo_root puxch vivado_ooc \
    puxch_20260901_hb${half_block}_hf${half_fft}]]

set syn_dcp [file join $build_dir puxch_ooc.dcp]
...
report_utilization -file [file join $build_dir puxch_impl_utilization.rpt]
report_utilization -hierarchical -file [file join $build_dir puxch_impl_utilization_hierarchical.rpt]
report_timing_summary -file [file join $build_dir puxch_impl_timing_summary.rpt]
report_route_status -file [file join $build_dir puxch_impl_route_status.rpt]
set impl_dcp [file join $build_dir puxch_impl.dcp]
write_checkpoint -force $impl_dcp

puts "INFO: puxch OOC implementation completed"
puts "INFO: implementation checkpoint: $impl_dcp"
puts "INFO: implementation utilization report: [file join $build_dir puxch_impl_utilization.rpt]"
