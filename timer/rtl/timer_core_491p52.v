...
module timer_core_491p52 #(
  ...
  // Note:
  // The clock frequency is 491.52 MHz, which is ~2.034 ns per clock cycle.
  // The timer increments depending on the state, which is used to correct
  // for fractional error of the counter.
  // For every 3 clock ticks, the counter increased by 1x TimerIncrement0
  // and 2x TimerIncrement1

  // Parameters

  // 11'b10_000010001, ~= 2.0332 ns
  localparam [10:0] TimerIncrement0 = 11'd1041;

  // 11'b10_000010010, ~= 2.0352 ns
  localparam [10:0] TimerIncrement1 = 11'd1042;
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      int_timer_state_reg <= int_timer_state_reg == 'd2 ? 'd0 : int_timer_state_reg + 1'd1;
    end
  end
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      int_timer_ns_frac_reg <= int_timer_ns_frac_reg + (int_timer_state_reg == 'd0 ? TimerIncrement0 : TimerIncrement1);
    end
  end
  ...
  assign int_timer_ns  = int_timer_ns_frac_reg[40:9];
